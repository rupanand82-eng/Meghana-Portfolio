...
export default function AITwinBot({ isOpen, onClose }: AITwinBotProps) {
  ...
  useEffect(() => {
    const greeting: ChatMessage = {
      id: "initial-greeting",
      role: "model",
      content: `Hello! 👋 I am the AI Twin of **Pallem Meghana Naga Rajeswari**. 

I can answer questions regarding my machine learning algorithms, specific full-stack Python projects (like the Automated Trading Bot or AI Health Assistant), current GPA (8.8) at Seshadri Rao Gudlavalleru Engineering College, plus certifications!

Feel free to ask me anything about my journey—or choose one of the suggest prompts below to test my credentials!`,
      timestamp: new Date()
    };
  ...
  const promptSuggestions = [
    "Tell me about your AI Health Assistant",
  ...
  const handleReset = () => {
    const greeting: ChatMessage = {
      id: "initial-greeting",
      role: "model",
      content: `Hello again! 👋 My AI Twin core is refreshed. Ask me about my Python programs, my GPA of 8.8, or my certifications. Let's talk code!`,
      timestamp: new Date()
    };
    setMessages([greeting]);
    setErrorStatus(null);
    ...
    <div className="fixed inset-y-0 right-0 z-50 w-full max-w-md sm:w-[420px] shadow-2xl transition-all duration-300 transform translate-x-0">
      ...
      <div className="flex h-full flex-col border-l border-slate-800 bg-slate-950/95 backdrop-blur-xl">
        ...
        <div className="bg-slate-900/80 px-4 py-1.5 border-b border-slate-800/40 flex items-center justify-between font-mono text-[9px] text-slate-400 uppercase tracking-wider">
          ...
          <span className="text-blue-400 font-bold">GPA 8.8</span>
        </div>
        ...
        <div className="border-t border-slate-800 p-3 bg-slate-950">
          ...
          >
            <input
              ...
              disabled={isLoading}
              className="flex-1 rounded-xl border border-slate-800 bg-slate-900/50 px-3.5 py-2 font-sans text-xs text-white placeholder-slate-500 outline-none focus:border-blue-500/50"
            />
            <button
              type="submit"
